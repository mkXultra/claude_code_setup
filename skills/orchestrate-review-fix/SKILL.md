...
description: Orchestrate a multi-model review-and-fix workflow using only mcp__acm__run and mcp__acm__wait. Use when Codex receives YAML input with fixed reviewer slots, a shared review prompt, and a required fixer model/session, and must run codex, glm5.1, and claude review slots, enforce structured reviewer output, merge and deduplicate findings, resolve contradictions by resuming reviewer sessions, optionally run a fixer, and return each agent's final model, session, and status.
---
